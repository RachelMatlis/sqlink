...
size_t virtIO_t::getLength() const
{
	if (m_fileP)
	{
		size_t currPos = ftell(m_fileP);//position of pointer
		fseek(m_fileP, 0, SEEK_END); //move file pointer to a specific position
		size_t fileLenght = ftell(m_fileP); 
		fseek(m_fileP, currPos, SEEK_SET);// needed for next read from last position of file
...
bool virtIO_t::setPosition(const size_t& position)
{
